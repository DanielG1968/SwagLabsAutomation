...
@allure.epic('Test invalid Checkout process')
@allure.id(2)
@allure.title("Trying to complete an order with invalid details")
@allure.description('Error message will appear')
@allure.severity(allure.severity_level.CRITICAL)
def test_add_to_cart_backpack(driver):
    login = Login(driver)
    PP = ProductPage(driver)
    ...
    with allure.step('Proceeding to the checkout section and completing the purchase'):
        cp.click_on_checkout_btn()
        cp.insert_invalid_first_name()
        cp.insert_invalid_last_name()
        cp.insert_invalid_zip_code()
        cp.click_on_continue_btn()
        cp.click_on_finish_btn()
        assert cp.is_error_message_displayed(), "Test failed: No error message displayed during checkout"
